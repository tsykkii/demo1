...
```
Разработанная информационная система предназначена для авторизации пользователей, управления учетными записями и обеспечения безопасности доступа к приложению.

Система реализована с использованием фреймворка Laravel и предоставляет следующие возможности:

авторизация пользователей;
выход из учетной записи;
обязательная смена пароля при первом входе;
подсчет количества неудачных попыток входа;
блокировка учетной записи после трех неверных попыток авторизации;
...
Система предусматривает разграничение доступа между пользователями и обеспечивает защиту учетных записей от подбора пароля.
```
...
| need_to_change_password | Признак обязательной смены пароля |
| wrong_counter | Количество неудачных попыток входа |
| is_ad | Признак администратора |
...
```
Метод выполняет аутентификацию пользователя и проверку введенных учетных данных.
```
...
проверка корректности введенного адреса электронной почты и пароля;
попытка авторизации пользователя;
проверка блокировки учетной записи;
проверка необходимости смены пароля;
увеличение счетчика неверных попыток входа;
блокировка учетной записи после трех неудачных попыток;
...
#### 3.2 Метод changePassword()

```
Метод предназначен для изменения пароля пользователя.
```
...
снятие признака обязательной смены пароля.

Возвращаемый результат:
Сообщение об успешном изменении пароля и переход на главную страницу.
```

---


#### 3.3 Метод createUser()

```
Метод предназначен для создания нового пользователя.
```
...
сохранение данных в таблице users.
...
```
Метод предназначен для изменения данных существующего пользователя.
```
...
```
Выполняемые действия:
проверка попытки удаления собственной учетной записи;
...
#### 3.6 Метод unlockUser()

```
Метод предназначен для блокировки и разблокировки учетной записи пользователя.
```
...
изменение значения счетчика wrong_counter;
блокировка пользователя при значении 3;
разблокировка пользователя путем обнуления счетчика.

Возвращаемый результат:
Сообщение об успешной блокировке или разблокировке пользователя.
```

#### 3.7 Метод logout()

```
Метод предназначен для завершения пользовательской сессии.
```
...
### 4. Представления системы

```
Для отображения пользовательского интерфейса используются Blade-шаблоны.

login.blade.php

Предназначен для авторизации пользователя.
...
Предназначен для отображения главной страницы приложения.

Содержит:

список пользователей;
информацию о блокировке;
информацию о необходимости смены пароля;
кнопки редактирования;
кнопки удаления;
кнопки блокировки и разблокировки пользователей;
кнопку создания нового пользователя;
кнопку выхода из системы.
user_form.blade.php

Предназначен для создания и редактирования пользователей.
...
```
В приложении реализована маршрутизация с использованием файла web.php.
...
Доступ к защищенным маршрутам осуществляется через middleware auth.

Для предотвращения доступа авторизованных пользователей к странице входа используется middleware guest.
```

### 6. Используемые технологии

```
При разработке информационной системы использовались следующие технологии:
